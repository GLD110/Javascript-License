          ...
          <table id="example2" class="table table-bordered table-hover">
            ...
            <?php $sno = 1; ?>
                ...
                <tr class="tbl_view text-center" >
                    ...
                    <td>
                        <a href="#" name="email" class="email" data-type="text" data-pk="<?= $row->id?>" data-url="<?php echo base_url( $this->config->item('index_page') . '/product/updateClickfunnels/email' ) ?>" data-title="Enter new Email"><?=$row->email ?></a>
                    </td>
                    <td>
                        <textarea disabled style="width: 100%; min-height: 70px; color: green;"><?PHP echo $row->scripts; ?></textarea>
                    </td>
                    <td>
                        <a href="#" class="is_active" data-name="<?=$row->email ?>" data-type="select" data-pk="<?= $row->id?>" data-url="<?php echo base_url( $this->config->item('index_page') . '/product/updateClickfunnels/is_active' ) ?>" data-title="Select status"><?php echo $row->is_active == '1' ? 'Active' : 'Inactive' ?></a>
                    </td>
...
<form method="POST" id='delfunnels' action="<?php echo base_url( $this->config->item('index_page') . '/product/delClickfunnels' ) ?>" >
    <input type="hidden" id = 'del_id' name="del_id" value=""/>
    <input type="hidden" id = 'del_email' name="del_email" value=""/>
</form>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form class="form-horizontal cus-form" id="Add_transaction" method="POST" action="<?php echo base_url( $this->config->item('index_page') . '/product/createClickFunnels' ) ?>"  data-parsley-validate>
